...
const GameRegistration = props => {
  ...
  const onChoose = e => {
    ...
    hero.name = user.name
    axios.post('/users/update', hero )
    .then(res => dispatch({type: 'SET_USER', payload: JSON.parse(res.config.data)}))
  ...
  return (
    <StyledGame>
            ...
            heroes.map(item => {
              return (
                <div key={item.class} className='card-class'>
                  ...
                  <img className='img-class' name={item.class} onClick={(e) => onChoose(e)} src={img[item.class]}></img>
                  <p>Hp: {item.hp}</p>
                  <p>Str: {item.str}</p>
                  <p>Dex: {item.dex}</p>
                  <p>Vit: {item.vit}</p>
      ...
      <div style={{fontWeight: 700, fontSize: '30px'}}>LOG IN or CREATE ACCOUNT !</div>
      }
    </StyledGame>
  )
}
